...
The first step is to deploy Redpanda, which can be done in multiple ways. See the [Install Redpanda guides](../../quickstart/) for more information.
...
:::note
This assumes you're in the root directory of an existing NodeJS project. See [Using NodeJS with Redpanda](../guide-nodejs) for one way to get started with a new NodeJS project and examples of connecting to Redpanda with a native library. 
:::
